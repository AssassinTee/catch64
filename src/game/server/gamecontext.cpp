...
void CGameContext::CreateHammerHit(vec2 Pos) {
	// create the event
	CNetEvent_HammerHit *pEvent = (CNetEvent_HammerHit *) m_Events.Create(NETEVENTTYPE_HAMMERHIT,
...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, int MaxDamage) {
	// create the event
	CNetEvent_Explosion *pEvent = (CNetEvent_Explosion *) m_Events.Create(NETEVENTTYPE_EXPLOSION,
...
void CGameContext::CreateSound(vec2 Pos, int Sound, int64 Mask) {
	...
	// create a sound
	CNetEvent_SoundWorld *pEvent = (CNetEvent_SoundWorld *) m_Events.Create(NETEVENTTYPE_SOUNDWORLD,
																			sizeof(CNetEvent_SoundWorld), Mask);
	if (pEvent) {
		pEvent->m_X = (int) Pos.x;
		pEvent->m_Y = (int) Pos.y;
...
void CGameContext::SendChat(int ChatterClientID, int Mode, int To, const char *pText) {
	...
	if (ChatterClientID >= 0 && ChatterClientID < MAX_CLIENTS) {
		if (Mode == CHAT_TEAM) {
			...
			str_format(aBuf, sizeof(aBuf), "%d:%d:%d:%s: %s", Mode, TeamID, ChatterClientID,
					   Server()->ClientName(ChatterClientID), pText);
		} else
			str_format(aBuf, sizeof(aBuf), "%d:%d:%s: %s", Mode, ChatterClientID, Server()->ClientName(ChatterClientID),
					   pText);
	} else
...
void CGameContext::SetKillerTeam(int ClientID, int Killer, bool silent) {
	...
	if (!m_apPlayers[TeamID] || m_apPlayers[ClientID]->GetTeamID() == TeamID)
		return;

	m_apPlayers[ClientID]->SetTeamID(TeamID);

	//copy skin
	copy_skin(m_apPlayers[ClientID]->m_TeeInfos,
			  m_apPlayers[Killer]->m_TeeInfos);//copy Teaminfos from killer, original Teeinfo may be changed from player TeamID
	...
	if (!silent) {
		...
		if (ClientID == TeamID) {
			...
			str_format(aBuf, sizeof(aBuf), "%s■■■^999 You are now in Team '%s' %s■■■", colorbuf,
					   Server()->ClientName(TeamID), colorbuf);
		}
...
void CGameContext::TestColor(int ColorID) {
	...
	char aBuf[128];
	str_format(aBuf, sizeof(aBuf), "ColorID: '%d', ColorName: '%s'", ColorID,
			   TeamHandler::getInstance().GetTeamName(ColorID).c_str());
...
void CGameContext::AbortVoteOnDisconnect(int ClientID) {
	if (m_VoteCloseTime && ClientID == m_VoteClientID && (str_startswith(m_aVoteCommand, "kick ") ||
														  str_startswith(m_aVoteCommand, "set_team ") ||
														  (str_startswith(m_aVoteCommand, "ban ") &&
														   Server()->IsBanned(ClientID))))
		m_VoteCloseTime = -1;
}
...
void CGameContext::OnTick() {
	...
	if (m_VoteCloseTime) {
		...
		if (m_VoteCloseTime == -1)
			...
			} else if (m_VoteEnforce == VOTE_ENFORCE_NO || (m_VoteUpdate && No >= (Total + 1) / 2) ||
...
void CGameContext::OnClientPredictedInput(int ClientID, void *pInput) {
	if (!m_World.m_Paused) {
		...
		if (m_NetObjHandler.ValidateObj(NETOBJTYPE_PLAYERINPUT, pInput, sizeof(CNetObj_PlayerInput)) == -1) {
			if (Config()->m_Debug && NumFailures != m_NetObjHandler.NumObjFailures()) {
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "NETOBJTYPE_PLAYERINPUT corrected on '%s'",
						   m_NetObjHandler.FailedObjOn());
				Console()->Print(IConsole::OUTPUT_LEVEL_DEBUG, "server", aBuf);
			}
		} else
...
void CGameContext::ApplyStartColors(int ClientID, CPlayer::TeeInfos &src) {
	...
	m_apPlayers[ClientID]->m_TeeInfosOriginal.m_aSkinPartColors[0] = TeamHandler::getInstance().GetNewBodyColor(
			ClientID);

	//Feet
	m_apPlayers[ClientID]->m_TeeInfosOriginal.m_aUseCustomColors[4] = 1;
	m_apPlayers[ClientID]->m_TeeInfosOriginal.m_aSkinPartColors[4] = TeamHandler::getInstance().GetNewFeetColor(
			ClientID);
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID) {
	...
	if (!pRawMsg) {
		if (Config()->m_Debug) {
			char aBuf[256];
			str_format(aBuf, sizeof(aBuf), "dropped weird message '%s' (%d), failed on '%s'",
	...
	if (Server()->ClientIngame(ClientID)) {
		if (MsgID == NETMSGTYPE_CL_SAY) {
			if (Config()->m_SvSpamprotection && pPlayer->m_LastChat &&
			...
			if (Length == 0 || (Config()->m_SvSpamprotection && pPlayer->m_LastChat &&
			...
			if (pMsg->m_Force) {
				...
				if ((Config()->m_SvSpamprotection &&
					 ((pPlayer->m_LastVoteTry && pPlayer->m_LastVoteTry + Server()->TickSpeed() * 3 > Now) ||
					  (pPlayer->m_LastVoteCall &&
					   pPlayer->m_LastVoteCall + Server()->TickSpeed() * VOTE_COOLDOWN > Now))) ||
			...
			if (str_comp_nocase(pMsg->m_Type, "option") == 0) {
				...
				if (SpectateID < 0 || SpectateID >= MAX_CLIENTS || !m_apPlayers[SpectateID] ||
					m_apPlayers[SpectateID]->GetTeam() == TEAM_SPECTATORS || SpectateID == ClientID)
					return;
			...
			if (pPlayer->GetTeam() == pMsg->m_Team ||
				(Config()->m_SvSpamprotection && pPlayer->m_LastSetTeam &&
				 pPlayer->m_LastSetTeam + Server()->TickSpeed() * 3 > Server()->Tick()) ||
			...
			if (m_pController->CanJoinTeam(pMsg->m_Team, ClientID) &&
				m_pController->CanChangeTeam(pPlayer, pMsg->m_Team)) {
			...
			if (Config()->m_SvSpamprotection && pPlayer->m_LastSetSpectatorMode &&
				pPlayer->m_LastSetSpectatorMode + Server()->TickSpeed() > Server()->Tick())
				return;
			...
			if (Config()->m_SvSpamprotection && pPlayer->m_LastEmote &&
				pPlayer->m_LastEmote + Server()->TickSpeed() * 3 > Server()->Tick())
				return;
			...
			else if (pPlayer->m_LastKill &&
					 pPlayer->m_LastKill + Server()->TickSpeed() * Config()->m_SvSelfkillCooldown > Server()->Tick()) {
				pPlayer->m_LastKillRequest = Server()->Tick();
				char aSelfkill[128];
				float seconds = (pPlayer->m_LastKill + Server()->TickSpeed() * Config()->m_SvSelfkillCooldown -
								 Server()->Tick() * 1.0f) / Server()->TickSpeed();
		...
		if (MsgID == NETMSGTYPE_CL_STARTINFO) {
			...
			while (pCurrent) {
				...
				int NumOptions = 0;
				for (CVoteOptionServer *p = pCurrent;
					 p && NumOptions < MAX_VOTE_OPTION_ADD; p = p->m_pNext, ++NumOptions);
...
void CGameContext::ConShuffleTeams(IConsole::IResult *pResult, void *pUserData) {
	...
	for (int i = 0; i < PlayerTeam; i++)
		pSelf->m_pController->DoTeamChange(pSelf->m_apPlayers[aPlayer[i]],
										   i < (PlayerTeam + rnd) / 2 ? TEAM_RED : TEAM_BLUE, false);
}
...
void CGameContext::ConAddVote(IConsole::IResult *pResult, void *pUserData) {
	...
	CVoteOptionServer *pOption = (CVoteOptionServer *) pSelf->m_pVoteOptionHeap->Allocate(
			sizeof(CVoteOptionServer) + Len);
	pOption->m_pNext = 0;
...
void CGameContext::ConchainSpecialMotdupdate(IConsole::IResult *pResult, void *pUserData,
...
void
CGameContext::ConchainSettingUpdate(IConsole::IResult *pResult, void *pUserData, IConsole::FCommandCallback pfnCallback,
...
void CGameContext::ConchainGameinfoUpdate(IConsole::IResult *pResult, void *pUserData,
										  IConsole::FCommandCallback pfnCallback, void *pCallbackUserData) {
	pfnCallback(pResult, pCallbackUserData);
	if (pResult->NumArguments()) {
		CGameContext *pSelf = (CGameContext *) pUserData;
...
void CGameContext::OnConsoleInit() {
	...
	Console()->Register("restart", "?i[seconds]", CFGFLAG_SERVER | CFGFLAG_STORE, ConRestart, this,
						"Restart in x seconds (-1 = abort)");
	...
	Console()->Register("set_team", "i[id] i[team] ?i[delay]", CFGFLAG_SERVER, ConSetTeam, this,
						"Set team of player to team");
	Console()->Register("set_team_all", "i[team]", CFGFLAG_SERVER, ConSetTeamAll, this,
...
void CGameContext::OnSnap(int ClientID) {
	...
	if (ClientID == -1 && Server()->DemoRecorder_IsRecording() &&
		...
		CNetObj_De_TuneParams *pTuneParams = static_cast<CNetObj_De_TuneParams *>(Server()->SnapNewItem(
				NETOBJTYPE_DE_TUNEPARAMS, 0, sizeof(CNetObj_De_TuneParams)));
		if (!pTuneParams)
			return;
...
const char *CGameContext::GameType() const {
	return m_pController && m_pController->GetGameType() ? m_pController->GetGameType() : "";
}
