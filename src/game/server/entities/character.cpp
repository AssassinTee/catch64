...
bool CCharacter::IsGrounded() {
	if (GameServer()->Collision()->CheckPoint(m_Pos.x + GetProximityRadius() / 2,
...
void CCharacter::HandleNinja() {
	...
	if (m_Ninja.m_CurrentMoveTime > 0) {
		...
		vec2 OldPos = m_Pos;
		GameServer()->Collision()->MoveBox(&m_Core.m_Pos, &m_Core.m_Vel,
		...
		// check if we Hit anything along the way
		{
			...
			int Num = GameWorld()->FindEntities(Center, Radius, (CEntity **) aEnts, MAX_CLIENTS,
...
void CCharacter::Tick() {
	...
	// handle hook mode
	if (m_ActiveWeapon == WEAPON_HAMMER && m_aWeapons[m_ActiveWeapon].m_Ammo == 0) {
		int hooked_player = m_Core.m_HookedPlayer;
		if (hooked_player >= 0 && GameServer()->GetPlayerChar(hooked_player) &&
			GameServer()->m_apPlayers[hooked_player]->GetTeamID() != m_pPlayer->GetTeamID())
			GameServer()->GetPlayerChar(hooked_player)->Die(m_pPlayer->GetCID(), WEAPON_GAME);
	}
...
bool CCharacter::TakeDamage(vec2 Force, vec2 Source, int Dmg, int From, int Weapon) {
	...
	if ((GameServer()->m_apPlayers[From] && GameServer()->m_apPlayers[From]->GetTeamID() == m_pPlayer->GetTeamID() &&
